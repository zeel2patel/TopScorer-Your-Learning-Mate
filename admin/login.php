    ...
    <div class="container-content">
      <form class="margin-t" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="POST">
        ...
        <?php if($_REQUEST['error']==1){?>
          <p class="error" style="color: orangered;font-size: 15px;" >User not active yet...</p>
        <?php }else if($_REQUEST['error']==2){?>
          <p class="error" style="color: orangered;font-size: 15px;" >User not exist.</p>
        <?php }else{?>
          <p class="error" style="color: orangered;font-size: 15px;" ></p>
        <?php }?>
        <div>◄ <a class="text-whitesmoke" style="" href="../index.php"><big>Back to Home</big></a> 
      </form>
    </div>
